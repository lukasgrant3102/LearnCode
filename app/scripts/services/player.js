angular.module('learnCodeApp')
  .service('Student', function (fbutil, _, $cookieStore) {
    // AngularJS will instantiate a singleton by calling "new" on this function
    var self = this, _so;

    self.getUniqId = function() {
    	// generate a unique idenftifier for the student and save it in a cookie to allow refreshes
    	if($cookieStore.get('studentID')) {
    		return self._id = $cookieStore.get('studentID');
    	} else {
    		$cookieStore.put('studentID', _.random(0,999999999)) ;
    		return self._id = $cookieStore.get('studentID');
    	}
    };
    ...
    self._connect = function() {
		...
		self.syncObject = fbutil.syncObject('sessions/' + self.PIN);
    	_so = self.syncObject;
    	return _so.$loaded()    	
    }
